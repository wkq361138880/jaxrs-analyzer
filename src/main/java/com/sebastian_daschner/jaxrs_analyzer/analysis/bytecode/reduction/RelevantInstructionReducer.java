...
/**
 * Determines the instructions, which are relevant for the return value of a
 * method by simulating a runtime stack with the byte code. This class is
 * thread-safe.
 *
...
public class RelevantInstructionReducer {
	...
	/**
	 * Returns all instructions which are somewhat "relevant" for the returned
	 * object of the method. The instructions are visited backwards - starting from
	 * the return statement. Load and Store operations are handled as well.
	 *
	 * @param instructions
	 *                         The instructions to reduce
	...
	private List<Instruction> reduceInstructionsInternal(final List<Instruction> instructions) {
		...
		while (!visitedInstructionPositions.containsAll(backtrackPositions)) {
			...
			final int backtrackPosition = backtrackPositions.stream()
					.filter(pos -> !visitedInstructionPositions.contains(pos)).findFirst()
					.orElseThrow(IllegalStateException::new);

			final List<Integer> lastVisitedPositions = stackSizeSimulator.simulateStatementBackwards(backtrackPosition);
			final List<Instruction> lastVisitedInstructions = lastVisitedPositions.stream().map(instructions::get)
			...
			final Set<Integer> unhandledLoadIndexes = findUnhandledLoadIndexes(handledLoadIndexes,
					lastVisitedInstructions);

			// for each load occurrence index -> find load/store backtrack positions
			// (reverse order matters here)
			final SortedSet<Integer> loadStoreBacktrackPositions = findLoadStoreBacktrackPositions(
					unhandledLoadIndexes);
	...
	/**
	 * Searches for load indexes in the {@code lastVisitedInstructions} which are
	 * not contained in {@code handledLoadIndexes}.
	 *
	 * @param handledLoadIndexes
	 *                                    The load indexed which have been handled
	 *                                    so far
	 * @param lastVisitedInstructions
	 *                                    The last visited instructions
	 * @return The unhandled load indexes
	 */
	private Set<Integer> findUnhandledLoadIndexes(final Set<Integer> handledLoadIndexes,
			final List<Instruction> lastVisitedInstructions) {
		final Set<Integer> lastLoadIndexes = InstructionFinder.findLoadIndexes(lastVisitedInstructions,
				RelevantInstructionReducer::isLoadIgnored);
	...
	/**
	 * Returns all backtrack positions of the given LOAD / STORE indexes in the
	 * instructions. The backtrack positions of both LOAD and store instructions are
	 * the next positions where the runtime stack size is {@code 0}.
	 *
	 * @param unhandledLoadIndexes
	 *                                 The LOAD/STORE indexes to find
	...
	private SortedSet<Integer> findLoadStoreBacktrackPositions(final Set<Integer> unhandledLoadIndexes) {
		...
		return unhandledLoadIndexes.stream()
				.map(index -> stackSizeSimulator
						.findLoadStoreBacktrackPositions(InstructionFinder.findLoadStores(index, instructions)))
				.collect(() -> set, Set::addAll, Set::addAll);
	}
