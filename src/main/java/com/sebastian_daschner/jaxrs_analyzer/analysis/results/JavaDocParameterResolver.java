...
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.BEAN_PARAM;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.CONTEXT;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.COOKIE_PARAM;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.DEFAULT_VALUE;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.FORM_PARAM;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.HEADER_PARAM;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.MATRIX_PARAM;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.PATH_PARAM;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.QUERY_PARAM;
import static com.sebastian_daschner.jaxrs_analyzer.model.Types.SUSPENDED;
...
/**
 * Resolves the actual {@code *Param} parameters analyzed by both the JavaDoc
...
final class JavaDocParameterResolver {
	...
	private static final String[] KNOWN_ANNOTATIONS = { PATH_PARAM, QUERY_PARAM, HEADER_PARAM, FORM_PARAM, COOKIE_PARAM,
			MATRIX_PARAM, DEFAULT_VALUE, SUSPENDED, CONTEXT };

	private JavaDocParameterResolver() {
		throw new UnsupportedOperationException();
	}

	static Optional<MemberParameterTag> findParameterDoc(final MethodParameter parameter,
			final MethodComment methodDoc) {
		return methodDoc.getParamTags().stream().filter(p -> hasAnnotation(parameter, p.getAnnotations())).findAny();
	}
	...
	static Optional<MemberParameterTag> findFieldDoc(final MethodParameter parameter, final ClassComment classDoc) {
		...
		return classDoc.getFieldComments().stream().filter(f -> hasAnnotation(parameter, f.getAnnotations())).findAny();
	}

	static Optional<MemberParameterTag> findRequestBodyDoc(final MethodComment methodDoc) {
		return methodDoc.getParamTags().stream().filter(p -> isRequestBody(p.getAnnotations())).findAny();
	}
	...
	private static boolean isRequestBody(final Map<String, String> annotations) {
		return annotations.entrySet().stream().noneMatch(e -> findKnownAnnotation(e.getKey()));
	}
