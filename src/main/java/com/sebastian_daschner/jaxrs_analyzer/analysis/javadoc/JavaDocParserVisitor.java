...
public class JavaDocParserVisitor extends VoidVisitorAdapter<Void> {
	...
	public void visit(ClassOrInterfaceDeclaration classOrInterface, Void arg) {
		...
		classOrInterface.getComment().filter(Comment::isJavadocComment).map(this::toJavaDoc)
	...
	public void visit(FieldDeclaration field, Void arg) {
		field.getComment().filter(Comment::isJavadocComment).map(this::toJavaDoc)
	...
	private void createFieldComment(Javadoc javadoc, FieldDeclaration field) {
		...
		classComment.getFieldComments()
				.add(createMemberParamTag(javadoc.getDescription(), field.getAnnotations().stream()));
	}

	@Override
	public void visit(MethodDeclaration method, Void arg) {
		method.getComment().filter(Comment::isJavadocComment).map(this::toJavaDoc)
	...
	private void recordMethodComment(Javadoc javadoc, MethodDeclaration method) {
		...
		Map<Integer, String> responseComments = createResponseComments(javadoc);
		methodComments.put(identifier, new MethodComment(comment, tags, responseComments, classComments.get(className),
				isDeprecated(javadoc)));
	}

	private List<MemberParameterTag> createMethodParameterTags(Javadoc javadoc, MethodDeclaration method) {
		return javadoc.getBlockTags().stream().filter(t -> t.getType() == JavadocBlockTag.Type.PARAM)
				.map(t -> createMethodParameterTag(t, method)).collect(Collectors.toList());
	}
	...
	private MemberParameterTag createMethodParameterTag(JavadocBlockTag tag, MethodDeclaration method) {
		Stream<AnnotationExpr> annotations = method.getParameterByName(tag.getName().orElse(null))
				.map(Parameter::getAnnotations).map(NodeList::stream).orElseGet(Stream::empty);
	...
	private MemberParameterTag createMemberParamTag(JavadocDescription javadocDescription,
			Stream<AnnotationExpr> annotationStream) {
		Map<String, String> annotations = annotationStream.filter(Expression::isSingleMemberAnnotationExpr)
				.collect(Collectors.toMap(a -> a.getName().getIdentifier(), this::createMemberParamValue));
	...
	private String createMemberParamValue(AnnotationExpr a) {
		...
		// throw new IllegalArgumentException(String.format("Javadoc param type (%s) not
		// supported.", memberValue.toString()));
	}
	...
	private Map<Integer, String> createResponseComments(Javadoc javadoc) {
		return javadoc.getBlockTags().stream()
				...
				.map(t -> t.getContent().toText()).map(ResponseCommentExtractor::extract).filter(Objects::nonNull)
				.collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
	}

	/**
	 * <b>Note:</b> This will not return the actual identifier but only the simple
	 * names of the types (return type &amp; parameter types). Doing a full type
	 * resolving with all imports adds too much complexity at this point. This is a
	 * best-effort approach.
	 */
	private MethodIdentifier calculateMethodIdentifier(MethodDeclaration method) {
		String[] parameters = method.getParameters().stream().map(p -> p.getType().asString())
				.map(p -> p.replace('.', '/')).toArray(String[]::new);
